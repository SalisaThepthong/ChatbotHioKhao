...
# Encrypt a message
def encrypt_message(f,message):
   
    encrypted_message = f.encrypt(message.encode())
    return encrypted_message

# Decrypt a message
def decrypt_message(f,encrypted_message):
...
def add_health_data():
    ...
    if user:
        key=genkey()
        user.key = key.decode()
        user.health = encrypt_message(token(key),health)
        db.session.commit()
    ...
    if customer_id:
        ...
        if recommended_food:
            ...
            if user:
                ...
                encrypted_message=user.health
                health = decrypt_message(token(user.key), encrypted_message)
